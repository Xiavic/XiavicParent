...
public class SimpleSignEditor extends AbstractSignEditor {
    ...
    @Override public void openUI(final @NotNull Player player, final @NotNull Sign sign) {
        ...
        // craft player handle
        final BlockPosition position = new BlockPosition.MutableBlockPosition(sign.getX(), sign.getY(),
            sign.getZ()); //Cast to double because of method overrides.
        // PREVIOUS: Gets the "PooledPosition"  of the sign for the packet editor
        // PRESENT: PooledBlockPosition no longer exists, so we just make instantiate new MutableBlockPosition,
